...
function ActivityTracker({ user, selectedDate }) {
  ...
  return (
    <div className="lc-card lc-card--activity">
      <h2 className="lc-title">Activity Log</h2>

      {suggestions.length > 0 && (
        <div className="lc-chips">
          {suggestions.map(s => (
            <button key={s} className="lc-chip lc-chip--activity" onClick={() => setName(s)}>
              {s}
            </button>
          ))}
        </div>
      ...
      <div className="lc-input-row">
        ...
        <input
          ...
          onKeyDown={e => e.key === 'Enter' && add()}
        />
        <button className="lc-add-btn lc-add-btn--activity" onClick={add}>Log</button>
      </div>
      ...
      {loading ? (
        ...
        <ul className="lc-items">
          {visible.length === 0 && (
            <li className="lc-empty">No activities logged for this day.</li>
          )}
          {visible.map(entry => (
            <li
              key={entry.id}
            ...
            >
              ...
              {entry.duration && (
                <span className="lc-badge lc-badge--activity">{entry.duration}</span>
              )}
